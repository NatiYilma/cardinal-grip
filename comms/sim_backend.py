...
logger = logging.getLogger("cardinal_grip.comms.sim")
...
class SimBackend(BaseBackend):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if num_channels != NUM_CHANNELS:
            logger.warning(
                "[SimBackend] num_channels=%d requested, simulator uses %d fixed channels.",
                num_channels,
                NUM_CHANNELS,
            )
        ...
        logger.debug(
            "SimBackend initialized (update_interval=%.3f, history_size=%d)",
            self.update_interval,
            history_size,
        )
    ...
    def start(self) -> None:
        ...
        if self._running:
            logger.debug("SimBackend.start() called, but simulation is already running.")
            return
        self._running = True
        self._thread = threading.Thread(target=self._run_loop, daemon=True)
        self._thread.start()
        logger.info("SimBackend simulation thread started.")
    ...
    def stop(self) -> None:
        ...
        if self._thread is not None:
            try:
                ...
                logger.exception("Error while joining SimBackend thread.")
            self._thread = None
        logger.info("SimBackend simulation thread stopped.")
        ...
        with self._lock:
            if head == "noise" and len(parts) >= 2:
                try:
                    ...
                    logger.info("SimBackend noise set to %d", self._jitter_amount)
                except ValueError:
                    logger.warning("SimBackend noise command ignored (bad value): %r", cmd)
            elif head == "reset":
                ...
                if self._history is not None:
                    self._history.clear()
                logger.info("SimBackend levels reset to LOW_LEVEL.")
            else:
                logger.debug("SimBackend send_command(%r) ignored (unknown).", cmd)
    ...
    def handle_char(self, ch: str, is_press: bool) -> None:
        ...
        # Avoid spamming logs for every key event; only debug if needed:
        logger.debug(
            "SimBackend key %r %s; pressed_keys=%r",
            c,
    ...
    def _run_loop(self) -> None:
        ...
        logger.debug("SimBackend run loop entering.")
        while self._running:
            now = time.time()
        ...
        logger.debug("SimBackend run loop exiting.")
