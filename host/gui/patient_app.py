...
class PatientWindow(QWidget):
    def __init__(
        ...
        # values[c] is a deque of samples for channel c (for plotting)
        self.values = [deque(maxlen=2000) for _ in range(NUM_CHANNELS)]
        self.times = deque(maxlen=2000)   # shared time axis

        # Session logging buffers (full-length, no truncation)
        self.logs_dir = os.path.join(PROJECT_ROOT, "data", "logs")
        os.makedirs(self.logs_dir, exist_ok=True)
        self.session_id: str | None = None
        self.session_wall_start: datetime | None = None
        self.session_logging_active = False
        self.log_times: list[float] = []
        self.log_values: list[list[int]] = [[] for _ in range(NUM_CHANNELS)]
        ...
        # ===== TARGET BAND (global band, feedback based on Ch0) =====
        band_group = QGroupBox("Target Zone (applies to Channel 0 for feedback)")
        band_layout = QHBoxLayout()
        band_group.setLayout(band_layout)
    ...
    def set_external_session_id(self, sid: str | None):
        """Set by dual launcher so game+patient logs share one session id."""
        self.external_session_id = sid

    def begin_session_logging(self):
        """Called by dual launcher at same moment game session starts."""
        ...
        self.session_logging_active = True
        self.log_times = []
        self.log_values = [[] for _ in range(NUM_CHANNELS)]

        # Reset plotting buffers for clean trace
        self.reset_session(clear_status=False)

    def end_session_logging(self):
        if not self.session_logging_active:
            return
        self.session_logging_active = False
    ...
    def _save_session_log(self):
        """Save current logged session to patient_session_<id>.csv + .json."""
        if not self.log_times or self.session_id is None:
            return

        csv_name = f"patient_session_{self.session_id}.csv"
        json_name = f"patient_session_{self.session_id}.json"
        csv_path = os.path.join(self.logs_dir, csv_name)
        json_path = os.path.join(self.logs_dir, json_name)

        duration_s = self.log_times[-1] if self.log_times else 0.0
        num_samples = len(self.log_times)
        ...
        try:
            with open(csv_path, "w", newline="") as f:
                ...
                header = ["time_s"] + [f"ch{c}_adc" for c in range(NUM_CHANNELS)]
                writer.writerow(header)
                for idx, t in enumerate(self.log_times):
                    row = [t]
                    for c in range(NUM_CHANNELS):
                        if idx < len(self.log_values[c]):
                            row.append(self.log_values[c][idx])
                        else:
                            row.append(0)
                    writer.writerow(row)
        except Exception:
    ...
    def handle_connect(self):
        # Shared backend from dual launcher
        if self.backend is not None and not self.owns_backend:
    ...
    def reset_session(self, clear_status: bool = True):
        for c in range(NUM_CHANNELS):
            self.values[c].clear()
        self.times.clear()
        self.start_time = time.time()
        ...
        # Also reset log buffers if active session logging wants a clean start
        if self.session_logging_active:
            self.log_times = []
            self.log_values = [[] for _ in range(NUM_CHANNELS)]
    ...
    def poll_sensor(self):
        ...
        # Session logging
        if self.session_logging_active:
            self.log_times.append(t)
            for c in range(NUM_CHANNELS):
                self.log_values[c].append(sample_vals[c])
    ...
    # ---------- CSV SAVING (manual button) ----------

    def save_csv(self):
        """
        Save the current session's *plot buffers*:
            time_s, ch0_adc, ch1_adc, ch2_adc, ch3_adc
        This is independent of the dual-session logging.
        """
        if not self.times:
            QMessageBox.information(self, "No data", "No samples to save yet.")
            return
        ...
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        default_name = f"patient_session_plot_{ts}.csv"
